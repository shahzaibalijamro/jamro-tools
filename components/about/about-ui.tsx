...
export function SectionHeading({
  ...
  return (
    <div className={`text-center ${className}`}>
      <h2 className="text-[24px] font-semibold leading-[1.2] text-[#111c2d] dark:text-on-surface md:text-[32px]">
        {title}
      </h2>
      {description ? (
        <p className="mx-auto mt-2 max-w-2xl text-[16px] leading-[1.6] text-[#434655] dark:text-on-surface-variant">
          {description}
        </p>
      ) : null}
    </div>
...
export function Panel({ children, className = "" }: PanelProps) {
  return (
    <div
      className={`rounded-[12px] border border-[#c3c6d7] dark:border-outline-variant bg-[#e7eeff] dark:bg-surface-container shadow-sm ${className}`}
    >
      {children}
    </div>
  );
